...
class CompressedDecoder(nn.Module):
    ...
    def _ancestor_skip(self, qt, emb_list, d, device):
        ...
        that ancestor's embedding. This gives spatially-local conditioning
        below the bottleneck instead of a meaningless global average.
        """
        if d <= self.d_bottle:
        ...
        # Compute ancestor key at d_bottle by right-shifting
        shift = 2 * (d - self.d_bottle)
        ancestor_keys = keys_d >> shift

        # Find where these ancestor keys sit in qt.keys[d_bottle]
...
def main():
    ...
    parser.add_argument('--d_bottle', type=int, default=3, help='Bottleneck depth (3 or 4)')
    ...
    print(f"=" * 70)
    print(f"Compressed Bottleneck AE  |  D_BOTTLE={D_BOTTLE}")
    print(f"Max nodes at bottleneck: {4**D_BOTTLE}  |  Max latent: {4**D_BOTTLE * 128} floats")
    print(f"Device: {device}")
    print(f"=" * 70)

    model = CompressedBottleneckAE(
        in_c=1, hidden=128, emb_dim=128,
    ...
    output_dir = f"plots/compressed_bottle{D_BOTTLE}_{timestamp}"
    ...
    print(f"Bottleneck: D={D_BOTTLE}, max {4**D_BOTTLE} nodes × 128 dims = {4**D_BOTTLE * 128} floats")
