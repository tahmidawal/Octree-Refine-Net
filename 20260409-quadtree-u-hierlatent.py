"""
20260409-quadtree-u-hierlatent.py

Option B: Hierarchical Compressed Latent AE for u only.

Goal: compress u into a small global latent z ∈ R^104 for use in
...
Architecture:
  Encoder (same bottom-up QuadConv as Feb18):
    - Produces per-node hidden states h[d] for each depth d
    - Per-depth mean pool: h[d] → mean over nodes → R^hidden
    - Per-depth compress: R^hidden → R^k  (k varies by depth)
    - Concatenate: z = [z[0]..z[7]] ∈ R^104
      depths 0-2: k=8  (3×8=24, coarse/global)
      depths 3-7: k=16 (5×16=80, fine/local)

  Decoder (top-down, no quadtree given):
    - z[d] is broadcast to all nodes at depth d
    - At each node: fuse(z[d] broadcast, positional_enc) → hidden
    - Predict split logit and value at each node
    - Nodes that split propagate children to next depth
    - Decoder builds tree autoregressively; at training time uses GT topology

  Latent dim: 104 total (small enough for Gauss-Newton ROM)
  Jacobian structure: block-diagonal (depth d nodes only depend on z[d])

Training:
  - Only trains model_u (not model_f)
  - Same Poisson pair data generation as Feb18
  - Same loss: MSE on leaf values + BCE on split decisions
"""
...
# Latent dims per depth: coarse depths get smaller slice, fine depths get larger
# depths 0,1,2 → k=8;  depths 3,4,5,6,7 → k=16  → total = 3*8 + 5*16 = 104
LATENT_DIMS = [8, 8, 8, 16, 16, 16, 16, 16]   # one entry per depth 0..7
LATENT_TOTAL = sum(LATENT_DIMS)                  # 104


class HierEncoder(nn.Module):
    """
    Bottom-up encoder that compresses each depth's node set into a fixed-size
    latent slice z[d] ∈ R^k_d via mean pooling + linear projection.

    Returns z ∈ R^104 (concatenation of all depth slices).
    Also returns per-node hidden states h[d] for use by the decoder skip connections.
    """
    def __init__(self, in_c=1, hidden=128, max_depth=7, pos_freqs=6,
        ...
        # Per-depth: pool nodes → single vector → compress to z[d]
        self.depth_pool_proj = nn.ModuleList([
            nn.Sequential(nn.Linear(hidden, hidden), nn.ReLU(), nn.Linear(hidden, latent_dims[d]))
            for d in range(max_depth + 1)
        ])
        self.depth_norm = nn.ModuleList([nn.LayerNorm(latent_dims[d]) for d in range(max_depth + 1)])

    def forward(self, qt):
        h = [None] * (self.max_depth + 1)

        # Initialize per-node features at each depth
        for d in range(self.max_depth + 1):
        ...
        # Per-depth mean pool → compress to z[d]
        z_slices = []
        for d in range(self.max_depth + 1):
            if h[d].numel() == 0:
                # No nodes at this depth — use zeros
                z_d = torch.zeros((1, self.latent_dims[d]), device=qt.device)
            else:
                mean_h = h[d].mean(dim=0, keepdim=True)          # (1, hidden)
                z_d = self.depth_pool_proj[d](mean_h)             # (1, k_d)
                z_d = self.depth_norm[d](z_d)
            z_slices.append(z_d)

        z = torch.cat(z_slices, dim=1).squeeze(0)   # (104,)
        return z, h   # z: global latent; h: per-node hidden states for decoder


class HierDecoder(nn.Module):
    """
    Top-down decoder that takes z ∈ R^104 and builds the tree autoregressively.

    At each depth d:
      - Broadcast z[d] (the depth-d slice of z) to all nodes at that depth
      - Fuse with positional encoding → hidden state
      - Predict: split logit (will this node split?) and leaf value
      - Expand children for nodes predicted/known to split

    At training time: uses GT tree topology from qt (teacher forcing on topology).
    At inference time (ROM): topology is built autoregressively by thresholding split logits.
    """
    def __init__(self, hidden=128, out_c=1, max_depth=7, pos_freqs=6,
        ...
        # Fuse: z_slice (broadcast) + pos → hidden
        self.fuse = nn.ModuleList([
            nn.Sequential(
                nn.Linear(latent_dims[d] + pos_dim, hidden),
                nn.ReLU(),
        ...
        # Latent slice offsets for indexing into z
        offsets = [0]
        for k in latent_dims[:-1]:
            offsets.append(offsets[-1] + k)
        self.register_buffer('latent_offsets', torch.tensor(offsets, dtype=torch.long))

    def _get_z_slice(self, z, d):
        """Extract z[d] slice and return as (1, k_d) tensor."""
        start = self.latent_offsets[d].item()
        end   = start + self.latent_dims[d]
        return z[start:end].unsqueeze(0)   # (1, k_d)

    def forward(self, qt, z):
        """
        qt   : Quadtree (used for GT topology at training time)
        z    : (104,) global latent vector
        """
        split_logits = [None] * self.max_depth
        val_pred     = [None] * (self.max_depth + 1)

        # Start top-down: initialize hidden state at depth 0
        N0 = len(qt.keys[0])
        device = qt.device

        h_prev = None   # will be set at depth 0 below
        ...
        for d in range(self.max_depth + 1):
            ...
            # Broadcast z[d] to all nodes at this depth
            z_slice = self._get_z_slice(z, d).expand(Nd, -1)   # (Nd, k_d)

            h = self.fuse[d](torch.cat([z_slice, pos], dim=1))  # (Nd, hidden)
            if d >= 1:
            ...
            # Propagate to children using GT topology (teacher forcing)
...
class HierLatentAE(nn.Module):
    """
    Full hierarchical compressed autoencoder.
    Encodes u quadtree → z ∈ R^104, decodes z → (split logits, val predictions).
    """
    def __init__(self, in_c=1, hidden=128, max_depth=7, pos_freqs=6,
                 latent_dims=LATENT_DIMS):
        super().__init__()
        self.encoder = HierEncoder(in_c=in_c, hidden=hidden, max_depth=max_depth,
                                   pos_freqs=pos_freqs, latent_dims=latent_dims)
        self.decoder = HierDecoder(hidden=hidden, out_c=in_c, max_depth=max_depth,
                                   pos_freqs=pos_freqs, latent_dims=latent_dims)
        self.latent_total = sum(latent_dims)
...
def generate_u_sample(noise_std=NOISE_STD, grad_threshold=0.3):
    """Generate one u quadtree sample."""
...
def compute_loss(model, qt, device, mse, bce, split_weight=0.5):
    ...
    # Small L2 on z to keep latent bounded (important for GN convergence)
    z_reg = 1e-4 * (z ** 2).mean()
...
def main():
    ...
    print(f"Option B: Hierarchical Latent AE  |  latent dim = {LATENT_TOTAL}")
    print(f"  Depth latent dims: {LATENT_DIMS}")

    model = HierLatentAE(in_c=1, hidden=128, max_depth=MAX_LEVEL,
                         pos_freqs=6, latent_dims=LATENT_DIMS).to(device)
    ...
    output_dir = f"plots/optionB_hierlatent_{timestamp}"
...
def _plot_samples(sample_buffer, model, output_dir, step):
    ...
    for row, samp in enumerate(sample_buffer):
        ...
        ax = axes[row, 3]
        z_np = z.numpy()
        ax.bar(range(len(z_np)), z_np, color='steelblue', alpha=0.7)
        ax.axhline(0, color='k', linewidth=0.5)
        ax.set_title(f"Latent z (dim={len(z_np)})\n|z|={z_np.std():.3f}", fontsize=9)
        ax.set_xlabel("dim"); ax.set_ylabel("value")
        # Draw vertical lines at depth boundaries
        offset = 0
        for d, k in enumerate(LATENT_DIMS[:-1]):
            offset += k
            ax.axvline(offset - 0.5, color='red', linewidth=0.8, linestyle='--', alpha=0.5)

    plt.suptitle(f"Option B: Hierarchical Latent AE — step {step}", fontsize=13)
    plt.tight_layout()
...
def _plot_losses(history, output_dir, step):
    ...
    plt.suptitle(f'Option B Training Losses — step {step}  (latent_dim={LATENT_TOTAL})', fontsize=12)
    plt.tight_layout()
...
def validate(model_path, output_dir, num_samples=10, device='cpu'):
    print(f"\n{'='*60}")
    print(f"Validation: Option B Hierarchical Latent AE")
    print(f"{'='*60}")
    ...
    for d, k in enumerate(LATENT_DIMS[:-1]):
        offset += k
        axes[1].axvline(offset - 0.5, color='yellow', linewidth=1, linestyle='--')

    # 3. z norm distribution
    ...
    plt.suptitle(f'Option B Validation Summary (latent_dim={LATENT_TOTAL})', fontsize=13)
    plt.tight_layout()
