...
class Quadtree:
    def __init__(self, max_depth=7, device='cpu'):
...
class QuadNode:
    ...
    def custom_patch_subdivide_concentric(self, k1, k2, cx, cy, inner_radius, outer_radius, 
                                            base_level=5, mid_level=6, max_level=7):
...
def validate_model(model_path, output_dir, num_samples=5, device='cpu'):
    ...
    with torch.no_grad():
        for i in range(num_samples):
            # Generate random concentric circle coordinates mapping to 5, 6, 7
            cx = random.uniform(0.25, 0.75)
            cy = random.uniform(0.25, 0.75)
            inner_radius = random.uniform(0.08, 0.12)  # Inner circle (level 7)
            outer_radius = random.uniform(0.20, 0.28)  # Outer circle (level 6 ring)
            ...
            ax1.axis('off')
            ax1.set_title(f"GT (Clean): k1={k1}, k2={k2}\nConcentric: L7 r={inner_r:.2f}, L6 r={outer_r:.2f} @ ({cx:.2f}, {cy:.2f})", fontsize=9)
...
if __name__ == "__main__":
    ...
    # !!! UPDATE THIS PATH TO YOUR NEWLY TRAINED MODEL BEFORE RUNNING !!!
    model_paths = [
        '/home/tahmid/Development/OctreeRefineNet/plots/YOUR_NEW_MODEL_DIR/best_model.pt',
    ]
