...
class Quadtree:
    ...
    def build_from_leaves(self, leaf_keys_by_depth, leaf_vals_by_depth):
        """
        leaf_keys_by_depth[d]: LongTensor (N_leaf_d,)
        leaf_vals_by_depth[d]: FloatTensor (N_leaf_d, C)
        """
        # Infer C_in
        C_in = None
        for d in range(self.max_depth + 1):
            if leaf_vals_by_depth[d].numel() > 0:
                C_in = leaf_vals_by_depth[d].shape[1]
                break
        if C_in is None:
            C_in = 1

        # 1) Initialize keys/features with leaves
        for d in range(self.max_depth + 1):
            lk = leaf_keys_by_depth[d].to(self.device).long()
            lv = leaf_vals_by_depth[d].to(self.device).float()
        ...
        # 3) Rebuild features_in[d] for expanded keys (keep leaf values, internal nodes = 0)
        for d in range(self.max_depth + 1):
            kd = self.keys[d]
        ...
        # 7) Build GT labels for Direction B
        for d in range(self.max_depth + 1):
            kd = self.keys[d]
            if kd.numel() == 0:
            ...
            self.values_gt[d] = self.features_in[d].clone()
...
class TreeEncoder(nn.Module):
    ...
    Bottom-up encoder with positional encoding:
      ...
      - repeatedly pools depth d -> d-1 and mixes via neighbor conv
    ...
    def forward(self, qt: Quadtree):
        ...
        for d in range(self.max_depth + 1):
            ...
            if fin is None or fin.numel() == 0:
                h[d] = torch.zeros((0, self.in_proj.out_features), device=qt.device)
                continue
        ...
        for d in range(self.max_depth, 0, -1):
            ...
            # Mix via neighbor conv (skip depth 0)
...
class TreeDecoderTeacherForced(nn.Module):
    """
    Teacher-forced top-down decoder with skip connections and positional encoding:
      - fuses decoder state with encoder skip features and position encoding
      - applies neighborhood mixing at each depth
    ...
    def __init__(self, hidden=64, out_c=1, max_depth=8, pos_freqs=6):
        ...
        # Neighborhood mixing at each depth after expansion
    ...
    def forward(self, qt: Quadtree, enc_h_list):
        ...
        for d in range(self.max_depth + 1):
            ...
            # Neighborhood mixing (helps a lot for spatial coherence)
...
# ==========================================
# 5. User's Adaptive Data Logic (unchanged tree creation)
# ==========================================
...
def generate_user_data():
    """
    TRUE adaptive output:
      leaf_keys_by_depth[d]: keys of leaves at depth d
      leaf_vals_by_depth[d]: their values
    ...
    for d in range(MAX_LEVEL + 1):
        if len(leaf_keys[d]) == 0:
            ...
            leaf_vals_by_depth.append(torch.tensor(leaf_vals[d], dtype=torch.float32))

    return leaf_keys_by_depth, leaf_vals_by_depth, leaves


# ==========================================
# 6. Main Execution (Direction B Training)
# ==========================================
...
def main():
    ...
    # 1) Generate TRUE adaptive leaves
    print("Generating Adaptive Leaves...")
    leaf_keys_by_depth, leaf_vals_by_depth, leaves = generate_user_data()
    total_leaves = sum([len(x) for x in leaf_keys_by_depth])
    print(f"Generated {total_leaves} leaves across depths.")

    # 2) Build TRUE quadtree format
    print("Building TRUE Adaptive Quadtree...")
    qt = Quadtree(max_depth=MAX_LEVEL, device=device)
    qt.build_from_leaves(leaf_keys_by_depth, leaf_vals_by_depth)
    print("Nodes per level:", [len(qt.keys[d]) for d in range(MAX_LEVEL + 1)])

    # 3) Model (Direction B) - with positional encoding and skip connections
    ...
    # Loss weighting: structure loss is easier, focus more on values
    split_weight = 0.2
    val_weight = 1.0

    # 4) Training: structure + leaf-value losses (more epochs for convergence)
    num_epochs = 1000
    print(f"Training Direction-B (split + leaf values) for {num_epochs} epochs...")
    losses = []
    for epoch in range(num_epochs):
        ...
        # Split loss across depths 0..MAX_LEVEL-1
        ...
        # Value loss: only supervise GT leaves, across all depths
        ...
        loss = split_weight * L_split + val_weight * L_val
        loss.backward()
        optimizer.step()

        losses.append(float(loss.item()))
        # Print loss every epoch
        print(f"Epoch {epoch}: total={loss.item():.6f}  split={L_split.item():.6f}  val={L_val.item():.6f}")
        ...
        if epoch % 50 == 0 or epoch == num_epochs - 1:
            with torch.no_grad():
                ...
                # Build predicted values dict keyed by (level, ix, iy)
                pred_vals_dict = {}
                for d in range(MAX_LEVEL + 1):
                    ...
                    for i in range(len(kd)):
                        pred_vals_dict[(d, int(ix[i].item()), int(iy[i].item()))] = pr[i]

            cmap = plt.get_cmap('viridis')

            fig, axes = plt.subplots(1, 3, figsize=(18, 6))

            # Plot 1: GT Quadtree with rectangles
            ax1 = axes[0]
            ax1.set_xlim(0, 1)
            ax1.set_ylim(0, 1)
            ax1.set_aspect('equal')
            ax1.axis('off')
            ax1.set_title(f"GT Quadtree (Level {MIN_LEVEL}-{MAX_LEVEL})\nFunction: sin(2π{K1}x)sin(2π{K2}y)", fontsize=12)

            for node in leaves:
                normalized_color = (node.val + 1) / 2  # Normalize from [-1,1] to [0,1]
            ...
            ax2.axis('off')
            ax2.set_title(f"Predicted Leaf Values (Epoch {epoch})", fontsize=12)
            ...
            for node in leaves:
                ...
                pred_val = pred_vals_dict.get((d, ix, iy), 0.0)
            ...
            plt.tight_layout()
            plt.savefig(f'quadnet_directionB_epoch{epoch:04d}.png', dpi=150, bbox_inches='tight')
            plt.close(fig)
            print(f"  -> Saved quadnet_directionB_epoch{epoch:04d}.png")

    print(f"Training complete. {len(leaves)} leaves.")
