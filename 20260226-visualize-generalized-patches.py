...
class QuadNode:
    ...
    def custom_patch_subdivide(self, k1, k2, cx, cy, radius, base_level=6, target_level=7):
        """
        Forces subdivision down to `base_level`. Then, conditionally subdivides
        to `target_level` only if the node is within `radius` of `(cx, cy)`.
        """
        ...
        # 1. Force everything to the base level
        if self.level < base_level:
            should_split = True
        else:
            # 2. Check if we are inside the circle for target level
            node_cx = self.x + self.size / 2
            node_cy = self.y + self.size / 2
            dist = np.sqrt((node_cx - cx)**2 + (node_cy - cy)**2)
            
            # Buffer ensures the patch fully encompasses the circle
            if dist <= radius + (self.size / 2):
                should_split = True
        ...
        if should_split:
            ...
            for child in self.children:
                child.custom_patch_subdivide(k1, k2, cx, cy, radius, base_level, target_level)
...
def generate_custom_patch_data(noise_std=NOISE_STD, cx=0.5, cy=0.5, radius=0.2, base_level=6, target_level=7):
    k1 = random.randint(1, 5)
    k2 = random.randint(1, 5)

    root = QuadNode(0, 0, 1.0, 0, target_level, base_level)
    root.custom_patch_subdivide(k1, k2, cx, cy, radius, base_level=base_level, target_level=target_level)

    leaves = []
    root.collect_leaves(leaves, k1, k2)

    leaf_keys = [[] for _ in range(target_level + 1)]
    leaf_vals = [[] for _ in range(target_level + 1)]
...
def validate_model(model_path, output_dir, num_samples=5, device='cpu'):
    ...
    with torch.no_grad():
        for i in range(num_samples):
            # Generate a random patch coordinate for this sample
            cx = random.uniform(0.2, 0.8)
            cy = random.uniform(0.2, 0.8)
            radius = 0.15
            
            leaf_keys_by_depth, leaf_vals_noisy, leaf_vals_clean, leaves, k1, k2 = generate_custom_patch_data(
                cx=cx, cy=cy, radius=radius, base_level=MIN_LEVEL, target_level=MAX_LEVEL
            )
            ...
            print(f"  Generated sample {i+1}/{num_samples}: k1={k1}, k2={k2}, Patch @({cx:.2f}, {cy:.2f})")
            ...
            ax1.axis('off')
            ax1.set_title(f"GT (Clean): k1={k1}, k2={k2}\nPatch Level 7 @ ({cx:.2f}, {cy:.2f})", fontsize=10)
...
if __name__ == "__main__":
    ...
    model_paths = [
        '/home/tahmid/Development/OctreeRefineNet/plots/20260226_175504/best_model.pt',
    ]
