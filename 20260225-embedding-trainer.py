...
def main():
    ...
    optimizer = torch.optim.AdamW(model_op.parameters(), lr=3e-4, weight_decay=1e-4)
    ...
    for step in range(num_steps):
        ...
        L_total.backward()
        torch.nn.utils.clip_grad_norm_(model_op.parameters(), max_norm=1.0)
        optimizer.step()
        scheduler.step()
        ...
        if step % 50 == 0:
            print(f"Step {step:4d} | Total Loss: {L_total.item():.5f} | Val: {L_val.item():.5f} | Split: {L_split_total.item()/nodes_counted:.5f} | lr: {scheduler.get_last_lr()[0]:.6f}")
