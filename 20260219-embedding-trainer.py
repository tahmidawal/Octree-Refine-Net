...
def main():
    ...
    for step in range(num_steps):
        ...
        # 2. Get Ground Truth Embeddings (No gradients needed for AEs)
        with torch.no_grad():
            E_f = model_f.encode(qt_f)
            E_u_target = model_u.encode(qt_u)
        ...
        L_emb_total = torch.tensor(0.0, device=device)
        L_split_total = torch.tensor(0.0, device=device)
        nodes_counted = 0
        
        # 3. Teacher Forcing: Query every actual node in the ground truth u tree
        for d in range(MAX_LEVEL + 1):
            kd = qt_u.keys[d]
            if kd.numel() == 0:
                continue
            ...
            # Loss 1: Predict exact latent vector
            L_emb_total += mse(emb_pred, E_u_target[d]) * len(kd)
            
            # Loss 2: Predict splits (up to MAX_LEVEL-1)
            if d < MAX_LEVEL and qt_u.split_gt[d] is not None:
                L_split_total += bce(split_logits, qt_u.split_gt[d]) * len(kd)
                
            nodes_counted += len(kd)

        L_total = (L_emb_total + 0.5 * L_split_total) / nodes_counted
        L_total.backward()
        optimizer.step()

        # Track loss history
        loss_history['total'].append(L_total.item())
        loss_history['emb'].append(L_emb_total.item()/nodes_counted)
        loss_history['split'].append(L_split_total.item()/nodes_counted)

        if step % 50 == 0:
            print(f"Step {step:4d} | Total Loss: {L_total.item():.5f} | Emb: {L_emb_total.item()/nodes_counted:.5f} | Split: {L_split_total.item()/nodes_counted:.5f}")
        ...
        if step > 0 and step % 100 == 0:
            ...
            if current_loss < best_loss:
                ...
                torch.save({
                    ...
                    'loss': current_loss,
                    'emb_loss': L_emb_total.item()/nodes_counted,
