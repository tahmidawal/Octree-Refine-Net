...
Modified to test Custom Topology (Base Level 6 + Patch Level 7)
"""
...
# ==========================================
# 5. Data Generation (Custom Patch Logic)
# ==========================================
...
class QuadNode:
    ...
    def custom_patch_subdivide_concentric(self, k1, k2, cx, cy, inner_radius, outer_radius, 
                                            base_level=6, mid_level=7, max_level=8):
        """
        Creates concentric circle topology:
          - Outside outer_radius: stops at base_level (level 6)
          - Between inner_radius and outer_radius: stops at mid_level (level 7)
          - Inside inner_radius: goes to max_level (level 8)
        """
        node_cx = self.x + self.size / 2
        node_cy = self.y + self.size / 2
        dist = np.sqrt((node_cx - cx)**2 + (node_cy - cy)**2)
        buffer = self.size / 2  # Buffer to ensure full coverage
        
        # Determine the target level for this node based on distance
        if dist <= inner_radius + buffer:
            # Inside inner circle -> go to max_level (8)
            target_level = max_level
        elif dist <= outer_radius + buffer:
            # In the ring between circles -> go to mid_level (7)
            target_level = mid_level
        else:
            # Outside outer circle -> stop at base_level (6)
            target_level = base_level
        
        # Stop if we've reached our target level
        if self.level >= target_level:
            return
        
        # Always split if below base_level, or if we haven't reached target
        should_split = self.level < target_level
        ...
        if should_split:
            ...
            for child in self.children:
                child.custom_patch_subdivide_concentric(k1, k2, cx, cy, inner_radius, outer_radius,
                                                        base_level, mid_level, max_level)
...
def generate_custom_patch_data(noise_std=NOISE_STD, cx=0.5, cy=0.5, 
                                inner_radius=0.1, outer_radius=0.25,
                                base_level=6, mid_level=7, max_level=8):
    """
    Generates concentric circle topology:
      - Outside outer_radius: level 6
      - Ring (inner to outer): level 7  
      - Inside inner_radius: level 8
    """
    k1 = random.randint(1, 5)
    k2 = random.randint(1, 5)

    root = QuadNode(0, 0, 1.0, 0, max_level, base_level)
    root.custom_patch_subdivide_concentric(k1, k2, cx, cy, inner_radius, outer_radius,
                                            base_level=base_level, mid_level=mid_level, max_level=max_level)
    ...
    return leaf_keys_by_depth, leaf_vals_noisy_by_depth, leaf_vals_clean_by_depth, leaves, k1, k2, inner_radius, outer_radius
...
def validate_model(model_path, output_dir, num_samples=5, device='cpu'):
    print(f"\n{'='*60}")
    print(f"Validating model with Custom Patch Analysis: {model_path}")
    print(f"{'='*60}")
    ...
    with torch.no_grad():
        for i in range(num_samples):
            # Generate random concentric circle coordinates for this sample
            cx = random.uniform(0.25, 0.75)
            cy = random.uniform(0.25, 0.75)
            inner_radius = random.uniform(0.08, 0.12)  # Inner circle (level 8)
            outer_radius = random.uniform(0.20, 0.28)  # Outer circle (level 7 ring)
            
            leaf_keys_by_depth, leaf_vals_noisy, leaf_vals_clean, leaves, k1, k2, inner_r, outer_r = generate_custom_patch_data(
                cx=cx, cy=cy, inner_radius=inner_radius, outer_radius=outer_radius,
                base_level=6, mid_level=7, max_level=MAX_LEVEL
            )
            ...
            print(f"  Generated sample {i+1}/{num_samples}: k1={k1}, k2={k2}, Center @({cx:.2f}, {cy:.2f}), inner_r={inner_r:.2f}, outer_r={outer_r:.2f}")
            ...
            ax1.axis('off')
            ax1.set_title(f"GT (Clean): k1={k1}, k2={k2}\nConcentric: L8 r={inner_r:.2f}, L7 r={outer_r:.2f} @ ({cx:.2f}, {cy:.2f})", fontsize=9)
...
if __name__ == "__main__":
    ...
    output_dir = f'/home/tahmid/Development/OctreeRefineNet/plots/validation_patch_{timestamp}'
    os.makedirs(output_dir, exist_ok=True)
    ...
    for model_path in model_paths:
        if os.path.exists(model_path):
            ckpt = validate_model(model_path, output_dir, num_samples=5, device=device)
            results.append({
                'path': model_path,
                'step': ckpt['step'],
