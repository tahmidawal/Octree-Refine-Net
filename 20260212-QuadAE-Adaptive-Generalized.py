...
def generate_user_data(noise_std=NOISE_STD):
    """
    ...
    k1 = random.randint(1, 7)
    k2 = random.randint(1, 7)
...
def main():
    ...
    # Training across many random trees (value loss only, no split loss)
    num_steps = 3000
    print(f"Training Denoising AE across random trees for {num_steps} steps...")
    print(f"  - Random K1,K2 in [1,7] each step")
    print(f"  - Noise std: {NOISE_STD}")
    print(f"  - Split loss: DISABLED")
    losses = []
    ...
    for step in range(num_steps):
        # Generate a NEW random tree each step
        ...
        # Value loss ONLY: supervise GT leaves with CLEAN values
        ...
        losses.append(float(L_val.item()))

        # Print loss every step
        print(f"Step {step}: val_loss={L_val.item():.6f}  k1={k1} k2={k2}  leaves={len(leaves)}")
        ...
        if step % 100 == 0 or step == num_steps - 1:
            ...
            ax_loss.set_ylabel('Value Loss (MSE)', fontsize=12)
            ax_loss.set_title(f'Training Loss Curve (Step {step})', fontsize=14)
            ax_loss.grid(True, alpha=0.3)
            if len(losses) > 100:
                # Add smoothed line
                window = min(50, len(losses) // 10)
                smoothed = np.convolve(losses, np.ones(window)/window, mode='valid')
                ax_loss.plot(range(window-1, len(losses)), smoothed, 'r-', linewidth=2, label=f'Smoothed (w={window})')
                ax_loss.legend()
            plt.tight_layout()
